...
DownloadApk {
    ...
    public static void downloadApk(Context context, String url, String title,final String appName) {
        ...
        //获取存储的下载ID
        long downloadId = (long) SPUtils.get(UIUtils.getContext(),DownloadManager.EXTRA_DOWNLOAD_ID,-1L);
//         downloadId = -1L;
        if(downloadId != -1) {
            //存在downloadId
    ...
    private static void start(Context context, String url, String title,String appName) {
        ...
        if(hasSDKCard()) {
            ...
            SPUtils.put(UIUtils.getContext(),DownloadManager.EXTRA_DOWNLOAD_ID,id);
        } else {
    ...
    public static void removeFile(Context context) {
        String filePath = (String) SPUtils.get(UIUtils.getContext(),"downloadApk",null);
        if(null != filePath) {
