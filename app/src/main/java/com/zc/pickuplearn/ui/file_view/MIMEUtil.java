 ...
 * @version 创建时间：2016-11-1 上午10:12:12 文件类型
 */
public class MIMEUtil {
    ...
    public void openFile(File file, Context context) {
        try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                uri = FileProvider.getUriForFile(context, "com.zc.pickuplearn.fileprovider", file);
                // 给目标应用一个临时授权
            ...
            // Intent.createChooser(inten, "请选择对应的软件打开该附件！");
        } catch (ActivityNotFoundException e) {
            Toast.makeText(context, "附件不能打开，请下载相关软件！",Toast.LENGTH_SHORT).show();
        }
    }

    public void openFileOnLine(String url, Context context) {
        Intent intent = new Intent();
        intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        // 设置intent的Action属性
        intent.setAction(Intent.ACTION_VIEW);
     ...
     *
     */
    public String getUrlMIMEType(String url) {
        String type = "*/*";
        String fName = url;
        // 获取后缀名前的分隔符"."在fName中的位置。
        int dotIndex = fName.lastIndexOf(".");
    ...
    public static MIMEUtil mimetutils;
